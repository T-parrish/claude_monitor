...
Rate limiting is handled separately: if the usage endpoint returns
HTTP 429, the request is retried with exponential backoff (15s, 30s, 60s,
honoring a `Retry-After` header when the server sends one). If it's still
rate-limited after that, the widget keeps showing the last good reading —
including its "Updated" timestamp — instead of an error, and tries again on
...
- `src/metrics.rs` — the `Metric` struct (label, percent, reset time) and
  the `MetricSource` trait: `fetch() -> Result<Vec<Metric>, FetchError>`.
  Return `FetchError::Failed` for real errors (shown as ⚠ in the dropdown)
  or `FetchError::RateLimited` when data is merely stale (the widget keeps
